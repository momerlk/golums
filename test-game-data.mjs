...
assert.deepEqual(trimPathToPlayer([[0,0],[10,0],[10,10]],8,1),[[8,0],[10,0],[10,10]]);
...
const keyed = new Uint8ClampedArray(3 * 3 * 4).fill(255); keyed[(1 * 3 + 1) * 4] = 0;
removeSpriteSheetBackground(keyed, 3, 3);
assert.equal(keyed[3], 0);
assert.equal(keyed[(1 * 3 + 1) * 4 + 3], 255);
const orange = new Uint8ClampedArray([255, 127, 39, 255]);
removeSpriteSheetBackground(orange, 1, 1);
assert.equal(orange[3], 0);
const orangeDetail = new Uint8ClampedArray(25 * 4); orangeDetail.set([255, 127, 39, 255], (2 * 5 + 2) * 4);
removeSpriteSheetBackground(orangeDetail, 5, 5);
assert.equal(orangeDetail[(2 * 5 + 2) * 4 + 3], 255);
